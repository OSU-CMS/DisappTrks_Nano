...
on_error() {
    ...
    find analysis_output -maxdepth 4 -type f -print || true
    exit "${status}"
}
trap on_error ERR

echo "Starting lxplus PocketCoffea job ${JOBID} on $(hostname)"
echo "Sandbox: $PWD"
...
cat > lxplus_inner_run_options.yaml <<'YAML'
...
runner_cmd=(
    ...
    --executor iterative
    --chunksize "${CHUNKSIZE}"
    --custom-run-options lxplus_inner_run_options.yaml
    -o "${JOB_OUTPUT}"
)
...
for output in "${outputs[@]}"; do
    ...
    local_output="analysis_output/${TAG}/${base}_job_${JOBID}.coffea"
    cp "${output}" "${local_output}"
    echo "Saved ${local_output}"

    if [ -n "${OUTPUT_DEST}" ]; then
        destination="${OUTPUT_DEST%/}/${TAG}/${base}_job_${JOBID}.coffea"
        if [[ "${destination}" == root://* ]]; then
            xrdcp -f "${local_output}" "${destination}"
        else
            mkdir -p "$(dirname "${destination}")"
            cp "${local_output}" "${destination}"
        fi
        echo "Copied ${local_output} to ${destination}"
    fi
done
