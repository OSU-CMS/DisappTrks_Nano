from disapptrks.schema import (
    ...
    JET_BASE_BRANCHES,
    JET_BRANCHES,
    JET_ID_RECONSTRUCTION_BRANCHES,
...
def test_jet_id_reconstruction_branches_can_replace_stored_jet_id():
    branches = (
        EVENT_ID_BRANCHES
        | ISOTRACK_BRANCHES
        | JET_BASE_BRANCHES
    ...
    report = audit_branches(branches)
    assert report.ready_for("search")


def test_stored_jet_id_can_replace_jet_id_reconstruction_branches():
    branches = (
        EVENT_ID_BRANCHES
        | ISOTRACK_BRANCHES
        | JET_BRANCHES
